...
import { formatDate, JOB_TYPES, cn } from '@/lib/utils'
...
export default function GraduatePlacementsPage() {
      ...
      const { data, error: qErr } = await supabase
        ...
        .select(
          ...
          *,
          ...
          profiles!placements_farm_id_fkey ( farm_name, farm_location ),
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-1 text-gray-600">
            Track training, attendance, and recruitment fees
          </p>
        </div>
        ...
        {loading ? (
          ...
          <div className="rounded-xl border border-gray-200 bg-white">
            <EmptyState
              ...
              description="When you are placed with a farm, details will show here."
            />
          </div>
        ) : (
          <ul className="space-y-4">
            {filtered.map((p) => {
              const job = p.jobs
              const farm = p.profiles
              const pay = firstPayment(p.payments)
              const showPayNow = pay?.status === 'pending'
              return (
                <li
                  key={p.id}
