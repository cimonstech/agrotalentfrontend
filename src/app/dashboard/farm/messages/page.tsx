  ...
  return (
    <div className={cn('flex', mine ? 'justify-end' : 'justify-start')}>
      ...
      >
        ...
        <p className={cn('mt-0.5 text-right text-[10px]', mine ? 'text-white/70' : 'text-gray-400')}>{formatDate(m.created_at, 'HH:mm')}</p>
      </div>
    </div>
  )
}
...
export default function FarmMessagesPage() {
  ...
  const loadConversations = useCallback(async () => {
    ...
    const gradIds = Array.from(new Set(list.map((c) => c.graduate_id)))
    if (gradIds.length > 0) {
      const { data: profs } = await supabase.from('profiles').select('id, full_name').in('id', gradIds)
      const nm: Record<string, string> = {}
      for (const p of profs ?? []) {
        const row = p as { id: string; full_name: string | null }
        nm[row.id] = row.full_name ?? 'Candidate'
      }
      setNameMap(nm)
    } else {
      setNameMap({})
  ...
  const visibleConversations = useMemo(() => {
    ...
    return conversations.filter((c) => {
      const name = (nameMap[c.graduate_id] ?? 'Candidate').toLowerCase()
        ...
        <Card className='flex h-full w-72 flex-shrink-0 flex-col overflow-hidden p-0'>
          <div className='border-b border-gray-50 p-4'>
            ...
            <div className='relative mt-2'>
              ...
              <input className='w-full rounded-xl border-0 bg-gray-50 py-2 pl-9 pr-3 text-sm outline-none' placeholder='Search conversations' value={search} onChange={(e) => setSearch(e.target.value)} />
            </div>
          </div>
          <div className='flex-1 overflow-y-auto'>
            {loading ? (
              ...
              visibleConversations.map((c) => {
                const name = nameMap[c.graduate_id] ?? 'Candidate'
                ...
                return (
                  ...
                  >
                    <div className='flex h-10 w-10 items-center justify-center rounded-xl bg-brand/10 text-sm font-bold text-brand'>{getInitials(name)}</div>
                    <div className='min-w-0 flex-1'>
                      <div className='flex items-center justify-between gap-2'>
                        <p className='truncate text-sm font-semibold text-gray-900'>{name}</p>
                        <p className='text-xs text-gray-400'>{c.last_message_at ? timeAgo(c.last_message_at) : ''}</p>
                      </div>
                      <p className='mt-0.5 truncate text-xs text-gray-500'>{latest ? truncate(latest.content, 44) : 'No messages yet'}</p>
                    </div>
            ...
            <>
              <div className='flex items-center gap-3 border-b border-gray-50 p-4'>
                <div className='flex h-10 w-10 items-center justify-center rounded-xl bg-brand/10 text-sm font-bold text-brand'>
                  {getInitials(nameMap[conversations.find((c) => c.id === selectedId)?.graduate_id ?? ''] ?? 'Candidate')}
                </div>
                <p className='font-semibold text-gray-900'>{nameMap[conversations.find((c) => c.id === selectedId)?.graduate_id ?? ''] ?? 'Conversation'}</p>
              </div>
              ...
              <div className='flex gap-3 border-t border-gray-50 p-4'>
                ...
                />
                <button type='button' className='flex h-10 w-10 items-center justify-center rounded-xl bg-brand text-white hover:bg-forest disabled:opacity-60' disabled={sending || !draft.trim()} onClick={() => void sendMessage()} aria-label='Send'>
